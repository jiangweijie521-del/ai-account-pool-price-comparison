...
- 每个商品提供 7/30/90 天价格与库存走势；桌面使用右侧抽屉，手机使用底部面板。
- 库存样本至少跨 2 天且趋势有效时，提供保守的预计消耗区间和置信度；最新记录超过 12 小时或数据不足时不预测。
...
- 历史趋势：SQLite 变化快照、6 小时心跳、90 天保留、真实时间比例横轴和最多 240 个绘图点的响应采样。
...
Worker 回退版对前端分析 POST 返回安全空响应，但不持久化统计或商品历史，也不提供 `/admin`；访问走势时会明确提示历史不可用。完整功能只存在于 Python + SQLite 部署。
...
- 商品历史：与匿名统计使用同一 SQLite 文件中的独立表，因此现有一致性备份会同时覆盖价格和库存走势。
