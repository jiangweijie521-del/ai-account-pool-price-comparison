...
- **Entry:** 每个商品行右侧使用独立的折线图标按钮；商品购买链接与走势按钮保持两个合法焦点目标。
- **Desktop:** 470px 右侧抽屉覆盖票据，保留页面作为暗化上下文；手机端改为 78dvh 的贴底面板。
- **Evidence order:** 当前/最低/最高价格、按真实时间比例绘制的价格与库存分区曲线、预计库存区间、样本证据和计算边界；单样本仍显示明确端点。
- **Motion:** 240ms 单向滑入；减少动态效果环境下立即完成。关闭后焦点返回原走势按钮。
- **Forecast:** 只用公开净库存下降趋势；补货后重新积累样本，最新记录超过 12 小时或数据不足时显示“暂不预测”。
